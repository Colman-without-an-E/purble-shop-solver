...
Something $\mathbf{M}_{rcrf}:= (m_{ij})$ $\in \text{M}_{256\times256}$ $(\{0,1,2,3,4\})$
where $m_{ij}$ is the number of right colour right features when $\mathbf{V}[i,:]$ is the true feature vector and $\mathbf{V}[j,:]$ is the guess feature vector. 
